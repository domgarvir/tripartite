...
axarr = axarr.ravel()
fontsize = 20
plot_boxplot_fromdf_wseaborn_to_ax(df,"r",box_by= "set_name",ax=axarr[0], index="B", ylabel= "r",points=True,y_lim=[-0.7,0.3])

plot_boxplot_fromdf_wseaborn_to_ax(df_rnd_zscore,"r",box_by= "set_name",ax=axarr[1],index="D", ylabel= "r(k)_Zscore_K", points=True, significance_lines=True)
axarr[1].tick_params(labelsize=14,labelrotation=30)

filename="../OUTPUT/Images/DDcorr_by_layer.pdf"
plt.tight_layout()
plt.savefig(filename)

#comparation between r in layers and in merged
# emp_df_filename="./OUTPUT/Data/SEM_df_%s.csv" % "RND"
# emp_df=pd.read_csv(emp_df_filename, index_col="name")
# df.set_index("name",inplace=True)
#
# df["r_merged"]=emp_df["rb_k"]
