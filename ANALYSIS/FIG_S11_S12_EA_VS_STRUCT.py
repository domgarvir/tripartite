...
for j in range(len(ext_MODES)): #ext_mode = col
    ...
    for i in range(len(str_metrics3)):
        if (i==0):
            ...
            title=""
        plot_regplot_fromdf_wseaborn_to_ax(emp_df, x=str_metrics3[i], y="Area_merged", ax=axarr[i][j],xlabel=str_metrics3[i], ylabel=ylabel, index=string.ascii_uppercase[index],title=title,corr_xy=corr_xy)
        index += 1
...
for j in range(len(ext_MODES)): #ext_mode = col
    ...
    for i in range(len(str_metrics2)):
        if (i==0):
            ...
            title=""
        plot_regplot_fromdf_wseaborn_to_ax(emp_df, x=str_metrics2[i], y="Area_merged", ax=axarr[i][j],xlabel=str_metrics2[i], ylabel=ylabel, index=string.ascii_uppercase[index],title=title,corr_xy=corr_xy)
        index += 1
